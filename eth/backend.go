...
func New(stack *node.Node, config *Config) (*Ethereum, error) {
	...
	fmt.Println("stateRoot  zw1", stateRoot.String())
	mystate, _ := eth.blockchain.StateAt(stateRoot)
	dump := mystate.RawDump(false, false, true).Accounts
	fmt.Println("len(dump) 999999 ", len(dump))
	//------------------------------------zhangwei-------------------------
	...
	fmt.Println("len(config.Genesis.Alloc)", len(config.Genesis.Alloc))
	...
	fmt.Println("len(genesisAlloc)", len(genesisAlloc)) // [] 说明没有初始化到stateDB
	//-----------------------------------------------------------------------------
