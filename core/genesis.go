...
func (g *Genesis) ToBlock(db ethdb.Database) *types.Block {
	...
	dump := statedb.RawDump(false, false, true).Accounts
	fmt.Println("len(dump) 777777 ", len(dump))
...
func (g *Genesis) ToBlock1(db ethdb.Database) (*types.Block,ethdb.Database) {
	if db == nil {
		db = rawdb.NewMemoryDatabase()
	}
	statedb, _ := state.New(common.Hash{}, state.NewDatabase(db), nil)
	...
	dump = statedb.RawDump(false, false, true).Accounts
	fmt.Println("len(dump)", len(dump))
	fmt.Println("root zzz ", root.String())
	return types.NewBlock(head, nil, nil, nil),db
}
...
func DefaultBaklavaGenesisBlock() *Genesis {
	...
	baklavaAlloc.UnmarshalJSON([]byte(mainnetAllocJSON1))
	return &Genesis{
