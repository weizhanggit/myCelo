...
type GovernanceParameters struct {
	UseMultiSig             bool         `json:"useMultiSig"` // whether the approver should be the multisig (otherwise it's the Admin_my)
...
type ValidatorsParameters struct {
	...
	Commission *fixed.Fixed `json:"commission"` // commission for genesis registered validator Groups_my
}
